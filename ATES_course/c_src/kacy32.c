...
double kacy_f32_main(double* _a, double* _b, double _sum, short size,
    ...
    for (int i = 0; i < size; i++) {
        ...
        int bx = EXPONENT(b[i]);
        bx += (bx==0);
        exp[i] = ax + bx - 127; //if ax+bx<127, exp[i]<0 this is wrong the resulting number is subnormal or zero, it should be 0
                                // actually   0 > exp[i] >= -23 the number is subnormal, exp[i] < -23 it is zero. however the test cases restricted them to be in the normal renge
        if (exp[i] < 0) {
            printf("one the cases where exp[i] < 0 i: %d, ax: %d, bx: %d, exp[i]: %d\n", i, ax, bx, exp[i]);
        }else if(exp[i] > 254) {printf("overflow case where  i: %d, ax: %d, bx: %d, exp[i]: %d\n", i, ax, bx, exp[i]);}
